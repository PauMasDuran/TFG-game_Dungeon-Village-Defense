...
@onready var projectile = load("res://Scenes/Mobs/magic_projectile.tscn")
...
func _ready():
	...
	$EnemyPowerLevel.actualize_power_level("MagicOrc",main.actual_dungeon_floor,auraType)
...
func _physics_process(delta):
	...
	if attackType == 1 and not attacking and not hurting and is_player_in_range():
		...
		attacking = true
		animateMagicAtk(orcActualDirection)
...
func animateMagicAtk(lastVelocity):
	...
	if angle >= -PI/8 and angle < PI/8:
		...
		$Sprite2D/AnimationPlayer.play("MagicAtk_down_right")
	elif angle >= 3 * PI/8 and angle < 5 * PI/8:
		$Sprite2D/AnimationPlayer.play("MagicAtk_down")
	elif angle >= 5 * PI/8 and angle < 7 * PI/8:
		$Sprite2D/AnimationPlayer.play("MagicAtk_down_left")
	elif angle >= 7 * PI/8 or angle < -7 * PI/8:
		$Sprite2D/AnimationPlayer.play("MagicAtk_left")
	elif angle < -5 * PI/8 and angle >= -7 * PI/8:
		$Sprite2D/AnimationPlayer.play("MagicAtk_up_left")
	elif angle < -3 * PI/8 and angle >= -5 * PI/8:
		$Sprite2D/AnimationPlayer.play("MagicAtk_up")
	elif angle < -PI/8 and angle >= -3 * PI/8:
		$Sprite2D/AnimationPlayer.play("MagicAtk_up_right")
